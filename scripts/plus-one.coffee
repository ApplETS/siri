...
# hubot who thanks me - show how many people thank me and why
# hubot (show ranking|ranking) - show top players

module.exports = (robot) ->
  robot.brain.data.achievements ||= {}

  robot.hear /(.*): *(\+1|thx|thanks) for (.*)$/i, (msg) ->
    receiver = msg.match[1].trim().replace("@", "")
    thanking = msg.message.user.name.replace("@", "")
    reason   = msg.match[3]
